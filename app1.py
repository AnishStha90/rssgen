...
def _get_categories(ld, soup, url="", site_name=""):
    # Return a single, reliable category (not a noisy grab-bag).
    # Structured data (articleSection) is the most trustworthy source since it's
    # set explicitly by the publisher's CMS for the article's actual section.
    cats = ld.get("articleSection", [])
    if isinstance(cats, str): cats = [cats]
    for c in cats:
        c = (c or "").strip()
        if not c or len(c) > 25: continue
        if c.lower() in _GENERIC_CAT_WORDS: continue
        if site_name and c.lower() == site_name.lower(): continue
        return [c]
    # Fall back to the category implied by the article's URL path (e.g. /opinion/...).
    url_cat = _category_from_url(url, site_name)
    if url_cat:
        return [url_cat]
    # Deliberately NOT falling back to meta "keywords"/"news_keywords" tags here:
    # on many news sites that field is populated with unrelated boilerplate/UI
    # text (e.g. "now", "win", "next") rather than real topical categories, which
    # produced garbage <category> tags in the generated feed.
    return []
...
def build_rss_xml(feed_row, items, base_url, feed_url_override=None):
    ...
    for item in items:
        ...
        if cats:                 lines.append(f'    <category>{cdata(cats[0])}</category>')
        ...
        if item["image"]:
            lines.append(f'    <enclosure url="{item["image"]}" type="{_img_mime(item["image"])}" length="0"/>')
...
def build_master_rss_xml(base_url):
    ...
    for _, item in combined:
        if item['_type'] == 'scraped':
            ...
            if img:
                lines.append(f'    <enclosure url="{img}" type="{_img_mime(img)}" length="0"/>')
